...
struct Args {
    ...
    #[arg(
        ...
        long,
        env = "DATABASE_URL",
        default_value = "sqlite://server/info.db"
    )]
    database_url: String,
}
...
async fn request_logging_middleware(request: Request, next: Next) -> Response {
    ...
    let client_ip = headers
        ...
        .or_else(|| headers.get("x-real-ip").and_then(|hv| hv.to_str().ok()))
        .unwrap_or("unknown");
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route(
            "/image",
            post(routes::image::post_image)
                .layer(DefaultBodyLimit::max(MAX_IMAGE_UPLOAD_SIZE as usize)),
        )
